...
fn create_client(config: &ProxyConfig) -> Client {
    ...
    if let (Some(cert_path), Some(key_path)) = (&config.client_cert_path, &config.client_key_path) {
        ...
        client_builder = client_builder.identity(
            reqwest::Identity::from_pem(&identity_data)
...
async fn proxy_handler(
    ...
    for (name, value) in req.headers() {
        if name.as_str().to_lowercase() != "host" && name.as_str().to_lowercase() != "connection" {
...
async fn main() -> std::io::Result<()> {
    ...
    let matches = ClapApp::new("pixy")
        ...
        .arg(
            Arg::with_name("client_cert_path")
                .long("client-cert")
                .value_name("FILE")
        ...
        .arg(
            Arg::with_name("max_payload_size")
                .long("max-payload")
                .value_name("BYTES")
    ...
    let max_payload_size = matches
        .value_of("max_payload_size")
        .unwrap()
        .parse::<usize>()
    ...
    let worker_count = matches
        ...
        .map(|count| {
            count
                .parse::<usize>()
                .expect("WORKER_COUNT must be a valid number")
        })
        .unwrap_or_else(|| num_cpus::get());

    info!(
        "Starting reverse proxy server on {}:{} with max payload size: {}MB and {} worker threads",
        server_host,
        server_port,
